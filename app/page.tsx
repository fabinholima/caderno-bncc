...
type Question = { id: string; code: string; statement: string; subject: string; grade: string; skill: string; difficulty: 'Fácil' | 'Média' | 'Difícil'; status: 'Aprovada' | 'Em revisão' | 'Rascunho'; alternatives: number; updatedAt: string };
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-[var(--canvas)] text-slate-950">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[252px] border-r border-slate-200 bg-[var(--navy)] text-white lg:flex lg:flex-col">
      ...
      <nav aria-label="Navegação principal" className="flex-1 px-3 py-6"><p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">Espaço de trabalho</p>{nav.map(([label, Icon]) => <button key={label} className={`mb-1 flex h-10 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-medium transition ${label === 'Questões' ? 'bg-white/12 text-white' : 'text-slate-300 hover:bg-white/7 hover:text-white'}`}><Icon className="size-[18px]" />{label}{label === 'Questões' && <span className="ml-auto rounded-full bg-[var(--lime)] px-2 py-0.5 text-[10px] font-bold text-[var(--navy)]">248</span>}</button>)}</nav>
